"""
Apteka (pharmacy) — cream plaster shopfront with green accents, ~1927.

Run standalone:
  blender --background --factory-startup --python generate.py -- \\
      --rule examples/apteka_pharmacy.py --output out/apteka_pharmacy.blend
"""
from pro import *

__version__ = "1.0.0"
__description__ = "1927 Polish apteka (pharmacy) shopfront"
__tags__ = ["poland", "1927", "pharmacy", "commercial"]
...
FACADE_COLOR = param("#e8efe4", group="Facade")
PLINTH_COLOR = param("#6e6254", group="Facade")
WOOD_COLOR = param("#2f5a3a", group="Facade")  # pharmacy-green door/trim
WINDOW_COLOR = param("#243038", group="Facade")
SHOP_GLASS = param("#3a5550", group="Facade")
CORNICE_COLOR = param("#d0c4b0", group="Facade")
ACCENT_COLOR = param("#2f6b45", group="Facade")
ROOF_COLOR = param("#9c3b28", group="Roof")
SOFFIT_COLOR = param("#e4ddce", group="Roof")
...
def FrontWall():
    ...
    shape.openings = [Opening(offset=0.4, width=1.15, height=DOOR_H, sill_height=0.0)]
    openings(WALL_H)
...
def RoomsOnly(clear_h):
    partition(
        ...
        thickness=0.12, margin=0.18, min_span=2.0, max_span=3.8,
        corridor=1.15, seed=17, door_width=0.9, door_height=2.1,
...
def SideFacade():
    split(
        ...
        UPPER_H >> split(x, flt(), 2.4 >> WindowBay(), flt()),
