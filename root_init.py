...
class Pro(bpy.types.Operator):
	...
	def invoke(self, context, event):
		...
		if ruleFile:
			...
			try:
				with GenerationSession(blender_context=context) as session:
					module, params = session.apply(ruleFile)
...
class Bake(bpy.types.Operator):
	...
	def execute(self, context):
		...
		try:
			with GenerationSession(blender_context=context) as session:
				highPolyParams = session.apply(ruleFile)[1]
		except Exception as e:
